...
From discussion forum I understand that random forest gave best result. This makes lots of sense, as random forests, together with boosting, are know to be the top performing algorithms for prediction (as applied in prediction competitions such as kaggle etc.).

From the below results of the random forest algorithm, I estimate 98% accuracy in the classification excercise.  

Cross validation is important for random forests because random forests are know to be prone to overfitting.  
Though I did not specify cross validation below, I get in results reference to what is applied under the hood: Resampling: Bootstrapped (25 reps), which refers to the bootstrapping as default cross validation in caret package.  To set more specifically cross validation I can specify the argument trControl e.g. trControl=trainControl(method="cv",number=5)

```{r random forests, cache=TRUE}
set.seed(1234)
mtryValues<-c(2)  # known from first not cached rf run that 2, 27, 52 are chosen
#modFit<-train(classe~., data=trainingtrain, method="rf",prox=TRUE,tuneGrid=data.frame(.mtry=mtryValues))
modFit<-train(classe~., data=trainingtrain, method="rf", prox=TRUE,tuneGrid=data.frame(.mtry=mtryValues))
# tuneLength=2 could be other try to limit computation (but results not to be quicker...)
# parallel=T
  

modFit
#getTree(modFit$finalModel,k=2)
head(getTree(modFit$finalModel,k=2))  # to limit output to 6 rows
...
Apply at the very end the best model to the testing dataset (using preObj built on training!)
